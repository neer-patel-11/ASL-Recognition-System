...
def task_download(**context):
    ...
    from data.download import download_kaggle_dataset  # ← removed load_train_csv
    ...
    # Push per-class counts for the email report (replaces old label_distribution from CSV)
    context["ti"].xcom_push(key="label_distribution", value=stats.get("class_counts", {}))
...
def task_validate(**context):
    ...
    # No csv_path needed — folder-based validation
...
def task_split(**context):
    ...
    from data.split import split_dataset  # ← updated module name

    ti           = context["ti"]
    dataset_path = ti.xcom_pull(key="dataset_path", task_ids="download_dataset")

    # No csv_path — folder-based split
    split_stats = split_dataset(dataset_path=dataset_path)
...
def task_send_report(**context):
    ...
    dl_classes= (download_stats or {}).get("class_count",       "N/A")
    ...
    <html><body style="font-family:Arial,sans-serif;color:#333;max-width:700px">
      ...
      <table border="1" cellspacing="0" style="border-collapse:collapse">
        ...
        <tr><td style="padding:4px 12px">Classes found</td><td style="padding:4px 12px">{dl_classes}</td></tr>
      ...
      <h3 style="color:#178c5a">📊 Label Distribution (from folder scan)</h3>
      <table border="1" cellspacing="0" style="border-collapse:collapse">
        <tr style="background:#f0f4ff"><th style="padding:4px 12px">Label</th><th style="padding:4px 12px">Count</th></tr>
        {label_rows}
      </table>

      <h3 style="color:#178c5a">📈 Baseline Stats (grayscale)</h3>
      <table border="1" cellspacing="0" style="border-collapse:collapse">
        <tr style="background:#f0f4ff"><th style="padding:4px 12px">Metric</th><th style="padding:4px 12px">Value</th></tr>
